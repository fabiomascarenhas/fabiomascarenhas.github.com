...
(define-syntax mdo
  (syntax-rules (<-)
    ((mdo op) op)
    ((mdo (x <- op) ops ...) (bind op (lambda (x) (mdo ops ...))))
    ((mdo op ops ...) (bind op (lambda (dummy) (mdo ops ...))))))
  ...
  (type-case PIAE iae
    ...
    [app (func arg)
         (let [(f (lookup-fundef func funs))]
           (mdo (arg <- (interp-exp arg funs env))
                (pret <- alloc)
                (interp-cmd (with (fundef-param f)
                                  (num arg)
                                  (fundef-body f))
...
(define (interp-cmd imp funs env)
  (type-case PWIMP imp
    [skip () (unit (void))]
    [st (lval rval)
        (mdo (val <- (interp-exp rval funs env))
             (loc <- (interp-exp lval funs env))
             (mset loc val))]
    [cif (cond true false)
         (mdo (val <- (interp-exp cond funs env))
              (if (= 0 val)
                  (interp-cmd false funs env)
                  (interp-cmd true funs env)))]
    [whl (cond body)
         (local [(define dowhile 
                   (mdo (val <- (interp-exp cond funs env))
                        (if (= 0 val)
                            (unit (void))
                            (mdo (interp-cmd body funs env)
                                 dowhile))))] dowhile)]
    [prt (exp)
         (mdo (val <- (interp-exp exp funs env))
              (begin (print val)
                     ...
                     (unit (void))))]
    [seq (cmd1 cmd2)
         (mdo (interp-cmd cmd1 funs env)
              (interp-cmd cmd2 funs env))]
    [with (var exp body)
          (mdo (val <- (interp-exp exp funs env))
               (free <- alloc)
               (mset free val)
               (interp-cmd body funs (env-entry var free env))
               pop)]
    [capp (func arg)
          (let [(f (lookup-fundef func funs))]
            (mdo (arg <- (interp-exp arg funs env))
...
(define (interp cmd vars funs)
  (local [(define env/count (init-env vars 1 (env-empty)))
    ...
    (first ((bind (mset 0 free)
                  (lambda (dummy)
                    (interp-cmd (parse-cmd cmd) funs env))) (mem-empty)))))

...
(define (interp exp env)
  (type-case EXP exp
    ...
    [add (left right)
         (mdo (val1 <- (interp left env))
              (val2 <- (interp right env))
    ...
    [deref (exp)
           (mdo (loc <- (interp exp env))
                (mget (numV-n loc)))]
    [skip () (unit (voidV))]
    [st (lval rval)
        (mdo (loc <- (interp lval env))
             (val <- (interp rval env))
             (mset (numV-n loc) val))]
    [cif (cond true false)
         (mdo (cval <- (interp cond env))
              (if (= 0 (numV-n cval))
                  (interp false env)
                  (interp true env)))]
    [whl (cond body)
         (local [(define dowhile (mdo (cval <- (interp cond env))
                                      (if (= 0 (numV-n cval))
                                          (unit (voidV))
                                          (mdo (interp body env)
                                               dowhile))))]
    ...
    [seq (cmd1 cmd2)
         (mdo (interp cmd1 env)
              (interp cmd2 env))]
    [ret (exp)
         (mdo (vret <- (interp exp env))
              (return vret))]
    [thr (tag exp)
         (mdo (vret <- (interp exp env))
              (throw tag vret))]
    [cth (tag exp)
         (mdo (vret <- (catch tag (interp exp env)))
              (unit vret))]
    [app (func arg)
         (mdo (varg <- (interp arg env))
              (parg <- alloc)
              (mset parg varg)
              (vclos <- (interp func env))
              (vret <- (catch 'retval
                         (interp (clos-body vclos) (env-entry (clos-param vclos)
                                                              parg
                                                              (clos-env vclos)))))
              (unit vret))]))
...
(define (eval cmd vars)
  (local [(define env/count (init-env vars 1 (env-empty)))
    ...
    (first ((mdo (mset 0 free)
                  (interp (parse cmd) env)) ;;(mem-empty)))))
